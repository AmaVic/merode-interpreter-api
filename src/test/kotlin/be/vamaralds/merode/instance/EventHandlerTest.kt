        ...
        with(eventHandler.model) {
            either {
                runBlocking {
                    ...
                    val expectedStoredObject =
                        Customer(0, State("Created", State.Type.Living), mapOf("name" to "George")).bind()
        ...
        with(eventHandler.model) {
            either {
                runBlocking {
                    ...
                    assert(
                        updatedObj.first().get<String>("name").bind() == "XYZ"
        ...
        with(eventHandler.model) {
            either {
                runBlocking {
                    ...
                    val updatedObj = eventHandler.handleEvent(event).bind()
                    assert(
                        updatedObj.first().get<String>("banReason").bind() == "Fraud"
                    ) { "Object was not updated correctly" }
                }
            }.mapLeft {
                assert(false) { "Event handling failed due to $it, but was expected to be successful" }
